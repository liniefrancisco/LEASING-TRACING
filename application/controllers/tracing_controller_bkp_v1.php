...
class Tracing_Controller extends CI_Controller {
    ...
    public function get_other_charges_arnt(){
        ...
        $invoices = $this->db_pms->query("
            ...
            WHERE
                (s.document_type = 'Invoice' OR s.document_type = 'Credit Memo')
            AND 
    ...
    public function get_payment_by_doc_nos(){
        ...
        $query = $this->db_pms->query("
            ...
            LEFT JOIN
                (SELECT s.*, s.gl_accountID as partnerID FROM subsidiary_ledger s where s.debit IS NULL AND s.credit IS NOT NULL AND s.document_type IN ('Payment', 'Payment Adjustment') GROUP BY s.doc_no, s.ref_no, s.credit) sl2
            ON
